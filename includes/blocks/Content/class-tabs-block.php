...
class Tabs_Block {
	...
	public function render_block( $attributes, $content, $block ) {
		return $this->render_tabs_html( $attributes, $content, $block );
	}
	 ...
	 * @return string Rendered block HTML.
	 */
	public function render_tabs_html( $attributes, $content, $block ) {
		// Extract attributes.
		...
		// Build HTML using static methods.
		$html = sprintf( '<div %s>', $wrapper_attributes );
		$html .= self::render_tabs_navigation( $tabs, $active_tab );
		$html .= '<div class="forjeon-tabs-content">';

		foreach ( $tabs as $index => $tab ) {
			$is_active = $index === $active_tab;
			$html .= self::render_tab_panel( $tab, $index, $is_active, $enable_accordion );
		}
